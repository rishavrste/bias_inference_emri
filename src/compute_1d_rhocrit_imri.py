"""
Compute shift-aggregated sigmas and 1D rho_crit for the IMRI 1PA case.

For each of the 25 IMRI grid points, 9 Fisher matrices are loaded from:
    fishers/mle/IMRI_1pa/shift_{s}/Fishers/Fisher_{i}.h5
    (computed by fisher_grid_optimized_shifts_1pa_imri_mle.py)

Parameter set (K=8 intrinsic, no extrinsic):
    ['m1', 'm2', 'a', 'p0', 'e0', 'Phi_phi0', 'Phi_r0', 'chi2']

Outputs (in bias_inference_emri/data/):
    sigmas/sigmas_1pa_K8_imri_all.npy      (25, 9, 8)
    sigmas/sigmas_1pa_K8_imri_median.npy   (25, 8)
    sigmas/sigmas_1pa_K8_imri_nominal.npy  (25, 8)
    rhocrit/rhocrit_1d_1pa_K8_imri.npy    (25,)
    rhocrit/rhocrit_1d_imri_summary.txt

Usage:
    python compute_1d_rhocrit_imri.py
"""
import os
import numpy as np
import h5py
...
FISHER_IMRI = os.path.join(FISHER_BASE, 'mle', 'IMRI_1pa')

SIGNAL_FILE = '/scratch/josh.mat/plot_scripts/data/signal/signal_parameter_array_IMRI.npy'
REC_FILE    = '/scratch/josh.mat/plot_scripts/data/recovered/mle/recovered_parameter_array_IMRI_1pa.npy'
...
def main():
    ...
    for i in range(N_GRID):
        ...
        sigma_all[i] = load_sigmas_for_point(FISHER_IMRI, i, m1, m2)

    NOMINAL = 4
    sigma_med = np.nanmedian(sigma_all, axis=1)   # (25, 8)
    sigma_nom = sigma_all[:, NOMINAL, :]           # (25, 8)

    np.save(os.path.join(SIGMA_DIR, 'sigmas_1pa_K8_imri_all.npy'),     sigma_all)
    np.save(os.path.join(SIGMA_DIR, 'sigmas_1pa_K8_imri_median.npy'),  sigma_med)
    np.save(os.path.join(SIGMA_DIR, 'sigmas_1pa_K8_imri_nominal.npy'), sigma_nom)
    print(f'\nSaved sigma arrays to {SIGMA_DIR}/')

    # 1D rho_crit
    rhocrit = np.full(N_GRID, np.nan)
    summary = os.path.join(RHOCRIT_DIR, 'rhocrit_1d_imri_summary.txt')
    with open(summary, 'w') as fout:
        hdr = (f'1D rho_crit — IMRI 1PA intrinsic-only Fisher at MLE recovery point\n'
    ...
    np.save(os.path.join(RHOCRIT_DIR, 'rhocrit_1d_1pa_K8_imri.npy'), rhocrit)
    print(f'\n1PA IMRI rho_crit: min={np.nanmin(rhocrit):.1f}  '
