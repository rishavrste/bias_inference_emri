"""
Compute shift-aggregated sigmas and 1D rho_crit for the IMRI_TAIL 0PA case.

For each of the 25 IMRI_TAIL grid points, 9 Fisher matrices are loaded from:
    fishers/mle/IMRI_TAIL_0pa/shift_{s}/Fishers/Fisher_{i}.h5
    (computed by fisher_grid_optimized_shifts_0pa_imri_tail_mle.py)

Parameter set (K=7 intrinsic, no extrinsic, no chi2):
    ['m1', 'm2', 'a', 'p0', 'e0', 'Phi_phi0', 'Phi_r0']

Outputs (in bias_inference_emri/data/):
    sigmas/sigmas_0pa_K7_imri_tail_all.npy      (25, 9, 7)
    sigmas/sigmas_0pa_K7_imri_tail_median.npy   (25, 7)
    sigmas/sigmas_0pa_K7_imri_tail_nominal.npy  (25, 7)
    rhocrit/rhocrit_1d_0pa_K7_imri_tail.npy     (25,)
    rhocrit/rhocrit_1d_imri_tail_0pa_summary.txt

Usage:
    python compute_1d_rhocrit_imri_tail_0pa.py
"""
import os
import numpy as np
import h5py
...
FISHER_TAIL_0PA  = os.path.join(FISHER_BASE, 'mle', 'IMRI_TAIL_0pa')

SIGNAL_FILE = '/scratch/josh.mat/plot_scripts/data/signal/signal_parameter_array_IMRI_TAIL.npy'
REC_FILE    = '/scratch/josh.mat/plot_scripts/data/recovered/mle/recovered_parameter_array_IMRI_TAIL_0pa.npy'

SIG_COLS    = [0, 1, 2, 3, 4, 11, 13]   # m1,m2,a,p0,e0,Phi_phi0,Phi_r0
REC_COLS    = [0, 1, 2, 3, 4, 11, 13]
...
def load_sigmas_for_point(fisher_base, idx, m1, m2):
    ...
    for s in range(N_SHIFTS):
        ...
        if not os.path.exists(fpath):
            print(f'    [MISSING] shift_{s} idx={idx}: {fpath}')
            continue
        try:
...
def main():
    ...
    for i in range(N_GRID):
        ...
        sigma_all[i] = load_sigmas_for_point(FISHER_TAIL_0PA, i, m1, m2)

    NOMINAL   = 4
    sigma_med = np.nanmedian(sigma_all, axis=1)
    sigma_nom = sigma_all[:, NOMINAL, :]

    np.save(os.path.join(SIGMA_DIR, 'sigmas_0pa_K7_imri_tail_all.npy'),     sigma_all)
    np.save(os.path.join(SIGMA_DIR, 'sigmas_0pa_K7_imri_tail_median.npy'),  sigma_med)
    np.save(os.path.join(SIGMA_DIR, 'sigmas_0pa_K7_imri_tail_nominal.npy'), sigma_nom)
    print(f'\nSaved sigma arrays to {SIGMA_DIR}/')

    rhocrit = np.full(N_GRID, np.nan)
    summary = os.path.join(RHOCRIT_DIR, 'rhocrit_1d_imri_tail_0pa_summary.txt')
    with open(summary, 'w') as fout:
        hdr = (f'1D rho_crit — IMRI_TAIL 0PA intrinsic-only Fisher at MLE recovery point\n'
    ...
    np.save(os.path.join(RHOCRIT_DIR, 'rhocrit_1d_0pa_K7_imri_tail.npy'), rhocrit)
    print(f'\nIMRI_TAIL 0PA rho_crit: min={np.nanmin(rhocrit):.1f}  '
